...
import { subscribePins, subscribeCheckRequests, subscribeHistoryPins, subscribeBusinesses } from "../services/supabaseService";
...
export default function MapPage(){
  ...
  useEffect(()=>{
    ...
    const init=()=>{
      const L=window.L;
      const map=L.map(mapRef.current,{center:[16.8409,96.1735],zoom:14,zoomControl:false,attributionControl:true,tap:true});
      ...
      setMapReady(true);
      if(!navigator.geolocation){ setGpsStatus("unavailable"); setShowGpsPopup(true); return; }
      navigator.geolocation.watchPosition(
        (pos)=>{
          setGpsStatus("granted"); setShowGpsPopup(false);
        ...
        (err)=>{ setGpsStatus(err.code===1?"denied":"unavailable"); setShowGpsPopup(true); },
  ...
  function confirmPickLocation(){
    ...
    const icon=L.divIcon({className:"",html:`<div style="display:flex;flex-direction:column;align-items:center">
      <div style="width:30px;height:30px;background:#534AB7;border-radius:50%;border:3px solid #fff;
        display:flex;align-items:center;justify-content:center;font-size:15px;
        box-shadow:0 4px 14px rgba(83,74,183,0.8)">📍</div>
    ...
    setPickedLocation(loc); setPickingLocation(false);
  ...
  // ── Subscriptions ─────────────────────────────────────────
  useEffect(()=>{ if(isRequiredToUpdate) return; return subscribePins(setPins); },[isRequiredToUpdate]);
  useEffect(()=>{ if(isRequiredToUpdate) return; return subscribeCheckRequests(setCheckRequests); },[isRequiredToUpdate]);
  useEffect(()=>{ if(isRequiredToUpdate) return; return subscribeBusinesses(setBusinesses); },[isRequiredToUpdate]);
  useEffect(()=>{
    if(isRequiredToUpdate) return;
    if(showHistory){ return subscribeHistoryPins(setHistoryPins, adminConfig?.pin_history_days??7); }
    else setHistoryPins([]);
      ...
      const icon = hasTip
        ? L.divIcon({className:"",
            html:`<div style="display:flex;flex-direction:column;align-items:center;cursor:pointer">
              <div style="background:linear-gradient(135deg,#e24b4a,#EF9F27);border-radius:20px;
                padding:3px 8px 3px 6px;border:2px solid rgba(255,255,255,0.8);
                box-shadow:0 3px 12px rgba(239,159,39,0.6);display:flex;align-items:center;
            ...
            </div>`,iconSize:[60,32],iconAnchor:[30,38]})
        : L.divIcon({className:"",
            html:`<div style="display:flex;flex-direction:column;align-items:center;cursor:pointer;opacity:${hist?0.5:1};">
              <div style="width:${hist?28:36}px;height:${hist?28:36}px;background:${hist?"rgba(40,40,40,0.9)":"rgba(20,20,20,0.92)"};
                border-radius:50%;border:2px solid ${hist?"rgba(255,255,255,0.15)":color};
                display:flex;align-items:center;justify-content:center;font-size:${hist?12:18}px;
                box-shadow:${hist?"none":"0 2px 12px "+color+"66"};">${emoji}</div>
            ...
            </div>`,iconSize:[hist?28:36,hist?28:44],iconAnchor:[hist?14:18,hist?14:44]});
      const m=L.marker([pin.lat,pin.lng],{icon}).addTo(mapInstance.current).on("click",()=>setSelectedPin(pin));
      markersRef.current.push(m);
    });
  },[pins,historyPins,mapReady,showHistory,isRequiredToUpdate,situationTypes]);
  ...
  useEffect(()=>{
    ...
    businesses.forEach(biz=>{
      const emoji = biz.business_categories?.emoji || "🏪";
      const html  = biz.cover_url
        ? `<div style="width:40px;height:40px;border-radius:50%;border:2.5px solid #fff;
            overflow:hidden;box-shadow:0 2px 14px rgba(0,0,0,0.7);cursor:pointer;background:#1a1a1a;">
            <img src="${biz.cover_url}" style="width:100%;height:100%;object-fit:cover;"
              onerror="this.style.display='none';this.parentElement.innerHTML='${emoji}'"/>
          </div>`
        : `<div style="width:40px;height:40px;border-radius:50%;border:2.5px solid #fff;
            background:#1a1a1a;display:flex;align-items:center;justify-content:center;
            font-size:20px;cursor:pointer;box-shadow:0 2px 14px rgba(0,0,0,0.7);">${emoji}</div>`;
      const icon = L.divIcon({className:"",html,iconSize:[40,40],iconAnchor:[20,20]});
      const m = L.marker([biz.lat,biz.lng],{icon}).addTo(mapInstance.current)
        .on("click",()=>setSelectedBusiness(biz));
      bizMarkersRef.current.push(m);
    });
  },[businesses,mapReady,showBusinesses]);

  // ── GPS controls ──────────────────────────────────────────
  function centerOnUser(){
    if(userLocation&&mapInstance.current) mapInstance.current.setView([userLocation.lat,userLocation.lng],16);
    else setShowGpsPopup(true);
  }
  ...
  function handleUpdateRedirect(){
    if(window.Telegram?.WebApp?.openLink) window.Telegram.WebApp.openLink(downloadUrl);
    ...
    <div style={{position:"relative",width:"100%",height:"100%",background:"#0d0d0d"}}>
      ...
      {accountType==="normal"&&!pickingLocation&&!isRequiredToUpdate&&(
        <div style={{position:"absolute",top:0,left:0,right:0,zIndex:600,
          ...
          <span style={{color:"#534AB7",fontSize:10,fontWeight:700,cursor:"pointer"}}
            onClick={()=>alert("Upgrade to Business.\nContact @dx0dev on Telegram.")}>
            Upgrade →
          </span>
        </div>
      )}
      ...
      {pickingLocation&&(<>
        <div style={{position:"absolute",top:0,left:0,right:0,zIndex:900,
          background:"rgba(83,74,183,0.97)",padding:"12px 16px 10px",boxShadow:"0 2px 20px rgba(0,0,0,0.5)"}}>
        ...
        <div style={{position:"absolute",bottom:0,left:0,right:0,zIndex:900,
          ...
          <button onClick={cancelPick} style={{flex:1,padding:"13px",borderRadius:12,
            border:"0.5px solid rgba(255,255,255,0.12)",background:"#1a1a1a",color:"#aaa",
      ...
      {!pickingLocation&&!isRequiredToUpdate&&(<>
        ...
        {/* Business layer toggle */}
        <div onClick={()=>setShowBusinesses(!showBusinesses)} style={{
          position:"absolute",top:accountType==="normal"?42:14,
          left:"50%",transform:"translateX(-50%)",
          background:showBusinesses?"rgba(83,74,183,0.95)":"rgba(20,20,20,0.95)",
          borderRadius:20,padding:"5px 14px",
          border:`0.5px solid ${showBusinesses?"rgba(83,74,183,0.5)":"rgba(255,255,255,0.08)"}`,
          zIndex:500,whiteSpace:"nowrap",cursor:"pointer",
          display:"flex",alignItems:"center",gap:6}}>
          <span style={{fontSize:13}}>🏪</span>
          <span style={{color:showBusinesses?"#fff":"#EF9F27",fontSize:11,fontWeight:600}}>
            {showBusinesses?`Shops ON · ${businesses.length}`:"Show Shops"}
          </span>
        </div>

        {/* History toggle */}
        <div onClick={()=>setShowHistory(!showHistory)} style={{
          position:"absolute",top:accountType==="normal"?82:54,
          left:"50%",transform:"translateX(-50%)",
        ...
        <div style={{position:"absolute",top:accountType==="normal"?90:60,right:12,zIndex:500}}>
          <button onClick={centerOnUser} style={{
            width:36,height:36,background:"rgba(20,20,20,0.96)",borderRadius:9,
            border:`0.5px solid ${gpsStatus==="granted"?"rgba(74,158,255,0.4)":gpsStatus==="denied"?"rgba(226,75,74,0.4)":"rgba(255,255,255,0.1)"}`,
            display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",position:"relative"}}>
            <i className="ti ti-navigation" style={{fontSize:16,
              color:gpsStatus==="granted"?"#4a9eff":gpsStatus==="denied"?"#e24b4a":"#ccc"}} aria-hidden="true"/>
            {gpsStatus!=="granted"&&(
              <div style={{position:"absolute",top:-3,right:-3,width:8,height:8,borderRadius:"50%",
                background:gpsStatus==="denied"?"#e24b4a":"#EF9F27",border:"1.5px solid #0d0d0d"}}/>
            )}
          </button>
        </div>
      ...
      {showGpsPopup&&!pickingLocation&&(
        <div onClick={()=>setShowGpsPopup(false)} style={{position:"fixed",inset:0,
          background:"rgba(0,0,0,0.7)",zIndex:2000,display:"flex",alignItems:"flex-end"}}>
          <div onClick={e=>e.stopPropagation()} style={{width:"100%",background:"#1a1a1a",
            ...
            {gpsStatus==="denied"?(
              <>
                <div style={{background:"rgba(226,75,74,0.08)",borderRadius:12,padding:"12px 14px",
                  marginBottom:16,border:"0.5px solid rgba(226,75,74,0.2)"}}>
                ...
                <button onClick={requestGpsAgain} style={{width:"100%",padding:"13px",borderRadius:12,
                  border:"none",background:"#4a9eff",color:"#fff",fontSize:14,fontWeight:700,
                  cursor:"pointer",fontFamily:"inherit",marginBottom:10}}>Try again</button>
              </>
            ):(
              <button onClick={requestGpsAgain} disabled={gpsStatus==="pending"} style={{
                ...
                background:gpsStatus==="pending"?"#333":"#4a9eff",color:"#fff",fontSize:14,
                fontWeight:700,cursor:gpsStatus==="pending"?"not-allowed":"pointer",fontFamily:"inherit"}}>
                {gpsStatus==="pending"?"Getting location...":"Enable GPS"}
              </button>
            )}
            <button onClick={()=>setShowGpsPopup(false)} style={{width:"100%",padding:"12px",
              borderRadius:12,background:"#222",border:"0.5px solid rgba(255,255,255,0.08)",
      ...
      {/* Popups */}
      {selectedBusiness&&<BusinessPopup business={selectedBusiness} onClose={()=>setSelectedBusiness(null)}/>}
      {selectedPin&&<PinPopup pin={selectedPin} onClose={()=>setSelectedPin(null)}/>}

      {/* Force update overlay */}
      {isRequiredToUpdate&&(
        <div style={{position:"fixed",inset:0,background:"rgba(13,13,13,0.98)",
          zIndex:9999,display:"flex",alignItems:"center",justifyContent:"center",padding:24}}>
          <div style={{width:"100%",maxWidth:340,background:"#1a1a1a",borderRadius:20,
            border:"0.5px solid rgba(255,255,255,0.09)",padding:"28px 24px",textAlign:"center",
            boxShadow:"0 10px 30px rgba(0,0,0,0.7)"}}>
            <div style={{width:64,height:64,borderRadius:"50%",background:"rgba(83,74,183,0.15)",
              border:"1.5px solid rgba(83,74,183,0.4)",display:"inline-flex",alignItems:"center",
              justifyContent:"center",fontSize:28,marginBottom:16}}>🚀</div>
            <div style={{color:"#fff",fontSize:18,fontWeight:700,marginBottom:8}}>Update Required</div>
            <div style={{color:"#aaa",fontSize:13,lineHeight:1.6,marginBottom:24}}>
              A new version is available. Please update to continue using the map.
            </div>
            <button onClick={handleUpdateRedirect} style={{width:"100%",padding:14,borderRadius:12,
              border:"none",background:"#534AB7",color:"#fff",fontSize:14,fontWeight:700,
              cursor:"pointer",fontFamily:"inherit",boxShadow:"0 4px 14px rgba(83,74,183,0.4)"}}>
              Update Now
            </button>
          </div>
        </div>
